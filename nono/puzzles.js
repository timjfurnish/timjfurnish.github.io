...
AddPuzzle("Roger?", 13, 17, "nOkM_w8IxAiE3pY3fW..P.8tNB_XMY3zJY37HxB3", "Object", 88)
...
function BuildButtonsForPuzzles(funcName)
{
	const output = []
	...
	for (var id of Object.keys(s_puzzles).sort(ByComplexity))
	{
		...
		const col = "rgb(" + Math.floor(100 + 155 * Math.sqrt(red)) + ", " + Math.floor(100 + 155 * Math.sqrt(green)) + ", 100)"
		output.push('<BUTTON CLASS="puzzleButton" STYLE="background-color: ' + col + '" onClick="' + funcName + '(\'' + id + '\')"><DIV class=buttonPuzzleName>' + FormatPuzzleNameAndSize(puzzle.name, puzzle.width, puzzle.height) + '</DIV><DIV STYLE="padding:5px">' + tagsHTML.join(" ") + '</DIV></BUTTON>')
	}
